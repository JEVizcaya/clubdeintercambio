...
def index(request):
    if(request.session.get('usuario_id')): 
        ...
        libros_disponibles = Libro.objects.exclude(usuario_id=request.session.get('usuario_id')).exclude(libro_id__in=Prestamo.objects.filter(estado_id=4).values('libro_id'))           
